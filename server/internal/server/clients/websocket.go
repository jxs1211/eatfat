...
func (c *WebSocketClient) PassToPeer(message packets.Msg, peerId uint64) {
	if peer, ok := c.hub.Clients.Get(peerId); ok {
...
func (c *WebSocketClient) SocketSendAs(message packets.Msg, peerId uint64) {
	select {
	case c.sendChan <- &packets.Packet{SenderId: peerId, Msg: message}:
	default:
