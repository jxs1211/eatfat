...
# Called during the physics processing step of the main loop. Physics processing means that the frame rate is synced to the physics
func _physics_process(delta: float) -> void:
	...
	if abs(velocity.angle_to(input_vec)) > TAU / 15: 
		...
		player_direction_msg.set_direction(velocity.angle()) # Returns this vector's angle with respect to the positive X axis
		WS.send(packet)
